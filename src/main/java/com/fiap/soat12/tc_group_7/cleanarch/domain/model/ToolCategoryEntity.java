package com.fiap.soat12.tc_group_7.cleanarch.domain.model;

import java.util.Objects;
import java.util.UUID;

public class ToolCategoryEntity {

    private final UUID id;
    private String toolCategoryName;
    private Boolean isActive;

    /* CONSTRUCTORS */
    public ToolCategoryEntity(String toolCategoryName) {
        if(toolCategoryName == null || toolCategoryName.isBlank()) {
            throw new IllegalArgumentException("A tool category name must not be null or blank");
        }

        this.id = UUID.randomUUID();
        this.toolCategoryName = toolCategoryName;
        this.isActive = true;
    }

    public ToolCategoryEntity(UUID id, String toolCategoryName, Boolean isActive) {
        this.id = id;
        this.toolCategoryName = toolCategoryName;
        this.isActive = isActive;
    }

    /* BUSINESS LOGIC METHODS */
    private void changeName(String newToolCategoryName) {
        if(Objects.isNull(newToolCategoryName)) {
            throw new IllegalArgumentException("New name cannot be null.");
        }
        this.toolCategoryName = newToolCategoryName;
    }

    public void deactivate() {
        if (Boolean.FALSE.equals(this.isActive)) {
            throw new IllegalStateException("Cannot deactivate an stock who is already inactive.");
        }
        this.isActive = false;
    }

    /* GETTERS AND SETTERS */
    public UUID getId() {return id;}

    public String getToolCategoryName() {return toolCategoryName;}

    public Boolean getActive() {return isActive;}

    // --- DEFAULT OBJECT METHODS (equals, hashCode, toString) ---
    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;
        ToolCategoryEntity toolCategoryEntity = (ToolCategoryEntity) o;
        return Objects.equals(id, toolCategoryEntity.id); // Equality is based on the unique ID
    }

    @Override
    public int hashCode() {
        return Objects.hash(id);
    }

    @Override
    public String toString() {
        return "Stock {" +
                "id=" + id +
                ", toolCategoryName='" + toolCategoryName + '\'' +
                ", isActive=" + isActive + '\'' +
                '}';
    }
}
